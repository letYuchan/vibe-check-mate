...
- `public/` 는 없지만 다음 중 **하나 이상 해당** 하면 `public/` 생성 + 복사:
  - `vite.config.*` 존재
  - `package.json` 의 `devDependencies` 또는 `dependencies` 에 `vite` 포함 (config 파일 없어도 vite 사용 중)
  - `app/layout.tsx` / `app/root.tsx` / `pages/_document.tsx` 중 하나 존재 (Next / Remix 관례)
  - `package.json` 의 deps 에 `next` · `@remix-run/*` 포함
- 위 전부 해당 없고 루트에 `index.html` 만 있는 경우 (plain HTML / static) → `index.html` 과 **같은 디렉터리** 에 복사해서 단순 정적 서버(`npx serve .` 등) 로 서빙 가능하게 만듦
