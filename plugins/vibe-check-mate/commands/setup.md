...
## 2. 기존 정적 검사 스크립트 확인
- `package.json`에 `lint` 스크립트가 있는지 확인한다.
- `package.json`에 `typecheck` 스크립트가 있는지 확인한다.
- 둘 중 하나라도 없으면 setup을 중단하고, 사용자가 프로젝트에 맞는 정적 검사 도구를 먼저 설정하도록 보고한다.
- 이 플러그인은 lint/typecheck 도구를 선택하거나 설치하지 않는다.

## 3. `create-pre-commit-hook` 스킬 호출
- `pnpm add -D husky` + `pnpm dlx husky init`
- package.json scripts에 병합: `check: "bash ./scripts/run-static-check-with-logs.sh"`
- `.husky/pre-commit` 교체:
  ```sh
  #!/bin/sh
